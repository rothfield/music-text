...
impl LilyPondGenerator {
    ...
    pub async fn generate_svg(&self, lilypond_source: &str) -> GenerationResult {
        ...
        let svg_url = format!("/temp_{}.svg", temp_id);
...
mod tests {
    ...
    async fn test_generator_creation() {
        let generator = LilyPondGenerator::new("test_output".to_string());
        assert_eq!(generator.output_dir, "test_output");
    }
    
    #[tokio::test]
    async fn test_invalid_lilypond_source() {
        let generator = LilyPondGenerator::new("/tmp".to_string());
        let result = generator.generate_svg("invalid lilypond code").await;
