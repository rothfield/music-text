            ...
            for line in &stave.lines {
                if let StaveLine::ContentLine(content_line) = line {
                    for content_element in &content_line.elements {
                        spans.extend(render_content_element(content_element));
                    }
                }
            }
        }
...
fn render_content_element(content_element: &ContentElement) -> Vec<Span> {
    match content_element {
        ContentElement::Beat(beat) => beat.elements.iter()
            .enumerate()
            .map(|(index, beat_element)| render_beat_element(beat_element, beat, index == 0))
            .collect(),
        ContentElement::Barline(barline) => vec![
            render_simple_element("barline", &barline.source, "|")
        ],
        ContentElement::Whitespace(_) => vec![],
    }
}

fn render_beat_element(beat_element: &BeatElement, beat: &crate::parse::model::Beat, is_first_in_beat: bool) -> Span {
    match beat_element {
        BeatElement::Note(note) => {
            let mut span = render_simple_element("note", &note.pitch_string.source,
                &note.pitch_string.source.value.as_ref().unwrap_or(&"?".to_string()));
        ...
        BeatElement::Dash(dash) => {
            let mut span = render_simple_element("dash", &dash.source, "-");
            add_beat_context_attributes(&mut span, beat, is_first_in_beat);
            span
        },
        BeatElement::BreathMark(breath) => render_simple_element("breath", &breath.source, "'"),
    }
}

fn render_simple_element(span_type: &str, source: &crate::parse::model::Source, content: &str) -> Span {
...
fn add_note_specific_attributes(span: &mut Span, note: &crate::parse::model::Note) {
    ...
    if let Some(duration) = &note.duration {
        let num = *duration.numer().unwrap_or(&1u64);
        let den = *duration.denom().unwrap_or(&4u64);
        span.data_attributes.insert("data-duration".to_string(), format!("{}/{}", num, den));
    }
}
        ...
        let beat_char_length: usize = beat.elements.iter().map(|element| {
            match element {
                crate::parse::model::BeatElement::Note(n) => {
                    n.pitch_string.source.value.as_ref().map(|v| v.chars().count()).unwrap_or(1)
                },
...
/// Convert spans to character styles for CodeMirror
pub fn render_codemirror_styles(spans: &[Span], original_input: &str) -> Vec<CharacterStyle> {
    spans.iter().map(|span| {
...
pub fn render_codemirror(document: &Document, original_input: &str) -> (Vec<Span>, Vec<CharacterStyle>) {
