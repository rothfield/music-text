use crate::document_parser::{Document, Stave};
...
mod tests {
    use super::*;
    use crate::document_parser::parse_document_structure;

    #[test]
    fn test_parse_document_staves() {
        let input = "|1 2 3";
        let document = parse_document_structure(input).unwrap();
        let staves = parse_document_staves(document).unwrap();
        
        // Should return the same staves as input
        assert_eq!(staves.len(), 1);
        assert_eq!(staves[0].content_line.elements.len(), 6); // |, 1, space, 2, space, 3
    }
