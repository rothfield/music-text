            ...
            if let Some(stave) = element.get("Stave") {
                if let Some(lines) = stave.get("lines").and_then(|l| l.as_array()) {
                    for line in lines {
                        ...
                        if let Some(lyrics) = line.get("Lyrics").and_then(|l| l.as_array()) {
                            for syllable in lyrics {
                                if let Some(syl_obj) = syllable.get("Syllable") {
                                    if let Some(text) = syl_obj.get("text").and_then(|t| t.as_str()) {
                                        xml.push_str(&format!("<syllable>{}</syllable>", escape_xml(text)));
                                    }
                                }
                            }
                        }
