...
use crate::parser_v2_fsm::{Item, BeatElement};
...
pub fn generate_vexflow_js(
    ...
    // Collect all syllables from elements (including extras beyond notes)
    let all_syllables = collect_all_syllables_from_elements(elements);
    eprintln!("VEXFLOW DEBUG: Collected {} total syllables from elements", all_syllables.len());
...
// Add extra width when syllables are present for better spacing
if (window.vexflowSyllables && window.vexflowSyllables.length > 0) {{
    const syllableExtraWidth = window.vexflowSyllables.length * 30; // 30px extra per syllable
    minWidth += syllableExtraWidth;
    console.log('VexFlow Debug: Added ' + syllableExtraWidth + 'px for ' + window.vexflowSyllables.length + ' syllables, new minWidth=', minWidth);
}}
...
if (window.vexflowSyllables && window.vexflowSyllables.length > 0) {{
    ...
    // Use LilyPond approach: position syllables corresponding to notes/rests
    const syllables = window.vexflowSyllables;
    const actualNotes = vexNotes.filter(note => note.keys && note.getAbsoluteX);
    
    console.log('🎵 LilyPond approach: ' + syllables.length + ' syllables, ' + actualNotes.length + ' notes');
    
    if (actualNotes.length > 0 && syllables.length > 0) {{
        // Calculate spacing like LilyPond's \addlyrics
        const leftmostX = Math.min(...actualNotes.map(note => note.getAbsoluteX()));
        const rightmostX = Math.max(...actualNotes.map(note => note.getAbsoluteX() + (note.getWidth ? note.getWidth() : 40)));
        const totalWidth = rightmostX - leftmostX;
        
        if (syllables.length <= actualNotes.length) {{
            // Syllables <= notes: attach to corresponding note positions  
            console.log('🎵 Syllables <= notes, attaching to corresponding notes');
            
            syllables.forEach((syl, index) => {{
                if (index < actualNotes.length) {{
                    const note = actualNotes[index];
                    ...
                    context.restore();
                    console.log('🎵 Drew syllable "' + syl.text + '" at note ' + index + ', x=' + noteX);
                }}
            }});
        }} else {{
            // More syllables than notes: space evenly like LilyPond
            console.log('🎵 More syllables than notes, spacing evenly across measure');
            const syllableSpacing = totalWidth / Math.max(1, syllables.length - 1);
            
            syllables.forEach((syl, index) => {{
                const syllableX = leftmostX + (index * syllableSpacing);
                context.save();
                context.font = 'italic 0.8em Arial';
                context.textAlign = 'center';
                context.fillStyle = '#000';
                context.fillText(syl.text, syllableX, syllableY);
                context.restore();
                console.log('🎵 Drew spaced syllable "' + syl.text + '" at x=' + syllableX);
            }});
        }}
    }}
...
{}
        ...
        generate_notes_and_barlines_js(&measure.notes, &measure.barlines, &all_syllables)?,
...
/// Collect only actual syllables from FSM elements (no placeholders)
fn collect_all_syllables_from_elements(elements: &Vec<Item>) -> Vec<String> {
    let mut syllables = Vec::new();
    
    // Collect only real syllables, no placeholders
    for item in elements.iter() {
        if let Item::Beat(beat) = item {
            for beat_element in &beat.elements {
                if let Some(syllable) = beat_element.syl() {
                    if syllable != "_" {  // Skip melismatic extenders
...
fn generate_notes_and_barlines_js(notes: &[VexFlowNote], barlines: &[VexFlowBarline], all_syllables: &[String]) -> Result<String, String> {
    ...
    for (i, note) in notes.iter().enumerate() {
        ...
        // Note: syllables are now handled separately using all_syllables array
    ...
    // Add all syllables to window for rendering (using LilyPond's approach)
    if !all_syllables.is_empty() {
        js_lines.push(format!("console.log('🎵 Setting up {} total syllables using LilyPond approach');", all_syllables.len()));
        js_lines.push("if (!window.vexflowSyllables) window.vexflowSyllables = [];".to_string());
        
        for (i, syllable) in all_syllables.iter().enumerate() {
            if syllable != "_" {  // Only add non-placeholder syllables
                js_lines.push(format!(
                    "window.vexflowSyllables.push({{ syllableIndex: {}, text: '{}' }});",
                    i, syllable
                ));
            }
        }
