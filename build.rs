...
fn main() {
    println!("cargo:rerun-if-changed=grammar/systems.json");
    println!("cargo:rerun-if-changed=grammar/notation.pest.template");
    println!("cargo:rerun-if-changed=grammar/system-specific.template");
    ...
    println!("🎼 Generating music-txt grammar from templates...");

    // Read configuration and templates
    let config: Config = {
        let config_content = fs::read_to_string(&systems_config_path)
            .unwrap_or_else(|e| panic!("❌ Failed to read systems.json: {}", e));
        serde_json::from_str(&config_content)
            .unwrap_or_else(|e| panic!("❌ Failed to parse systems.json: {}", e))
    };

    let main_template = fs::read_to_string(&main_template_path)
        .unwrap_or_else(|e| panic!("❌ Failed to read main template: {}", e));

    let system_template = fs::read_to_string(&system_template_path)
        .unwrap_or_else(|e| panic!("❌ Failed to read system template: {}", e));
    ...
    for (system_name, system_config) in &config.systems {
        println!("📝 Generating rules for {} notation...", system_name);
    ...
    let should_write = if output_path.exists() {
        let existing_content = fs::read_to_string(&output_path)
            .unwrap_or_else(|e| panic!("❌ Failed to read existing output file: {}", e));
    ...
    if should_write {
        ...
        println!("✅ Grammar file {} is up to date", output_path.display());
    }
    
    // Show statistics
    let lines = final_grammar.lines().count();
    let systems = config.systems.len();
    println!("📊 Generated {} lines for {} notation systems", lines, systems);
    
    // List generated systems
    println!("🎵 Notation systems:");
    for (name, system_config) in &config.systems {
        println!("   • {}: {}", name, system_config.description);
    }
}