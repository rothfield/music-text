    ...
    for (let i = 0; i < elements.length; i++) {
        ...
        switch (element.type) {
            ...
            case 'Tuplet':
                ...
                console.log('🎵 Processing Tuplet:', element);
                const tupletResult = processTupletAdvanced(element, context);
                console.log('🎵 Tuplet result:', tupletResult);
                if (tupletResult.notes.length > 0) {
                    ...
                    console.log('🎵 Created VexFlow Tuplet object:', tuplet);
                    tuplet.setContext(context);
                    tuplets.push(tuplet);
                    console.log('🎵 Added tuplet to tuplets array, total:', tuplets.length);
...
function createAdvancedVexFlowNote(element) {
    ...
    if (element.accidentals && element.accidentals.length > 0) {
        element.accidentals.forEach((acc, index) => {
            // Handle both object format {accidental: "#", index: 0} and simple string format "#"
            if (typeof acc === 'string') {
                // Simple string format from our backend
                note.addModifier(new Accidental(acc), 0);
            } else if (acc.accidental) {
                // Object format (legacy or other sources)
                note.addModifier(new Accidental(acc.accidental), acc.index || 0);
            }
        });
    }
