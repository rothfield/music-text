...
async fn parse_text(Query(params): Query<HashMap<String, String>>) -> impl IntoResponse {
    ...
    match crate::process_notation(&input) {
        Ok(result) => {
            ...
            let rhythm_items = {
                let mut all_beats: Vec<crate::parse::model::Beat> = Vec::new();
                        ...
                        for line in &stave.lines {
                            if let crate::parse::model::StaveLine::ContentLine(content_line) = line {
                                for content_element in &content_line.elements {
                                    if let crate::parse::model::ContentElement::Beat(_beat) = content_element {
                                        // Note: beats are now parse::model::Beat, not rhythm::Item
                                        // For backward compatibility, we could convert or skip this
                                    }
                                }
                            }
                        }
                ...
                None // Temporarily disabled until we decide how to handle the type change
            };
