        ...
        match result {
            Ok(processing_result) => {
                ...
                self.output = match format {
                    ...
                    OutputFormat::JSON => {
                        serde_json::to_string_pretty(&processing_result.parsed_document)
                            .unwrap_or_else(|e| format!("JSON error: {}", e))
                    },
                    OutputFormat::Debug => {
                        format!("{:#?}", processing_result.parsed_document)
                    },
                    OutputFormat::SVG => {
                        processing_result.vexflow_svg.clone()
                    },
                    OutputFormat::Tokens => {
                        let spans = music_text::tree_functions::generate_syntax_spans(&processing_result.parsed_document, &self.input);
                        serde_json::to_string_pretty(&spans).unwrap_or("Spans not available".to_string())
                    },
                    OutputFormat::Document => {
                        serde_json::to_string_pretty(&processing_result.rhythm_analyzed_document)
                            .unwrap_or_else(|e| format!("JSON error: {}", e))
                    },
                    OutputFormat::CharacterStyles => {
                        let normalized_elements = music_text::tree_functions::generate_normalized_elements(&processing_result.rhythm_analyzed_document, &self.input);
                        let (_spans, styles) = music_text::tree_functions::generate_spans_and_styles(&normalized_elements);
